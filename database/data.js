...
var generateReservations = function() {
    var data = {};
    data.restaurantId = faker.random.number({"min": 1, "max": 1000000});
    data.tableNumber = faker.random.number({"min": 1, "max": 4});
    data.date = faker.date.between('2019-04-27', '2019-05-27').toISOString().slice(0, 10);
...
var generateTables = function() {
  var tables = [];
  for (var restaurantId = 1; restaurantId <= 1000000; restaurantId++) {
    for (var i = 1; i <= 4; i++) {
        var data = {};
